...
class DiffusionBackend:
    ...
    def _init_distributed(args):
        ...
        cfg_size = 2 if args.infer.diffusion.guidance_scale > 1 else 1
        ...
        # )
        # DiffusionBackend.ip_port_list = get_world_group().gather_all_rank_ip_port()

        # DiffusionBackend.pp_stage = (
        #     global_rank
        #     % (world_size // non_expert_data_parallel_size)
        #     // model_parallel_size
        # )
        # DiffusionBackend.pp_end_stage = (
        #     world_size // non_expert_data_parallel_size - 1
        # ) // model_parallel_size
        # DiffusionBackend.pp_main_rank = (
    ...
    def _init_cache_manager(
        ...
        kv_cache_kvargs = DiffusionBackend._get_kv_cache_params(args, attn_backend_type)
    ...
    def _init_linear_attn_cache_manager(
        ...
        return PagedKVCacheManager(
            ...
            num_hot_req=(args.infer.max_reqs + args.infer.dp_size - 1)
            // args.infer.dp_size,
            shape_per_token_dict=DiffusionBackend._get_linear_attn_cache_params(args),
            block_size=1,
            num_blocks=args.infer.num_blocks if num_blocks is None else num_blocks,
            device=local_rank,
            lazy_mode=True,
    ...
    def _build_and_setup_model(args, attn_backend):
        ...
        if not args.debug.skip_model_load:
            ...
            with torch.device("meta"):
                model = DiffusionBackend._build_model_architecture(args, attn_backend)
        ...
        if args.infer.op_impl != "cpu":
            model.apply(DiffusionBackend._move_one_module_to_device)
    ...
    def _build_model_architecture(args, attn_backend):
        ...
        if args.models.type == "hf-qwen3-next":
            model = DiffusionBackend.build_model(
                args.models,
                DiffusionBackend.cache_manager,
                ...
                linear_attn_cache=DiffusionBackend.linear_attn_cache_manager,
            )
        elif (
            args.models.type == "deepseek-v3"
            and DiffusionBackend.indexer_cache_manager is not None
        ):
            model = DiffusionBackend.build_model(
                args.models,
                DiffusionBackend.cache_manager,
                ...
                op_impl=args.infer.op_impl,
                mla_absorb=args.infer.mla_absorb,
                indexer_cache=DiffusionBackend.indexer_cache_manager,
            )
        else:
            model = DiffusionBackend.build_model(
                args.models,
    ...
    def _load_checkpoint(model, args):
        ...
        if args.models.type == "deepseek-v3" and args.models.quant_config.type in [
            ...
            if args.models.type == "llama":
                ...
                checkpoint = DiffusionBackend._remove_prefix(checkpoint, "model.")
                ...
                checkpoint = DiffusionBackend._load_hf_checkpoint(model, args)
            else:
    ...
    def _load_hf_checkpoint(model, args):
        ...
        return DiffusionBackend._remove_prefix(params, "model.")
    ...
    def build(args):
        ...
        # DiffusionBackend.processor = DiffusionBackend._init_processor(args)
        ...
        if args.models.type == "hf-qwen3-next":
            ...
            DiffusionBackend.cache_manager = DiffusionBackend._init_cache_manager(
                args,
                attn_backend_type,
                layer_filter_fn=filter_full_attn_layer,
                num_blocks=num_full_attn_blocks,
            )
            DiffusionBackend.linear_attn_cache_manager = DiffusionBackend._init_linear_attn_cache_manager(
                args,
            ...
            DiffusionBackend.indexer_cache_manager = DiffusionBackend._init_indexer_cache_manager(args)
        else:
            DiffusionBackend.cache_manager = DiffusionBackend._init_cache_manager(args, attn_backend_type)
            DiffusionBackend.cache_type = args.infer.cache_type
...
def load_gguf_deepseek_v3_gguf(
    ...
    model.apply(
        functools.partial(
            DiffusionBackend._move_one_module_to_device,
