...
class CacheManager:
    ...
    def enable_cache_for_model(
        ...
        def cached_forward(*args, **kwargs):
            ...
            # 为了让 TeaCacheBlocks 能够像 teacache_origin.py 那样直接计算 e 和 e0，
            # 我们需要让 model 保存 t（timestep），这样 TeaCacheBlocks 就可以访问它
            # 提取 t 参数（可能在 args 或 kwargs 中）
            t = None
            if len(args) > 1:
            ...
            # 注意：TeaCacheBlocks 会直接使用 get_cfg_group().rank_in_group 来判断是否是 condition 分支
            # 所以不需要保存 is_cfg
            if t is not None:
                model._last_timestep = t
            
            # 使用 mock.patch 临时替换 blocks
            # 根据 cache-dit 的实现方式，blocks 替换后，原始 forward 会调用替换后的 blocks
            # TeaCacheBlocks.forward 会自然接收到原始 forward 传递的所有参数（包括 e，即 e0）
